...
const ArraySection = ({ section, data, addItem, removeItem, updateItem, isDark }) => {
  ...
  return (
    <div className="space-y-3 sm:space-y-4">
        ...
        <div key={index} className={`p-3 sm:p-4 rounded-lg border ${
          ...
          <div className="flex justify-between items-center mb-3">
            <h4 className="font-semibold text-sm sm:text-base">
              {safeSection.charAt(0).toUpperCase() + safeSection.slice(1)} #{index + 1}
            </h4>
            <button
              onClick={() => removeItem(safeSection, index)}
            ...
            {Object.keys(item || {}).map((field) => (
              <div key={field}>
                ...
                {field === 'description' || field === 'achievements' ? (
                  <textarea
                    value={item[field] || ''}
                    onChange={(e) => updateItem(safeSection, index, field, e.target.value)}
                    rows={3}
      ...
      >
        + Add {safeSection.charAt(0).toUpperCase() + safeSection.slice(1)}
      </button>
    </div>
  );
};
